...
func _ready():
	...
	my_init(wall.takeNewId(), "ButtonHide")
	if templ2D.assoc_left:
		pass
	if templ2D.assoc_right:
		var other = get_parent().get_parent().get_parent().lhand.get_node("Menu/ButtonHide")
...
# warning-ignore:return_value_discarded
	self.connect("clicked", self, "_b_clicked")
	
func _b_clicked(_left):
